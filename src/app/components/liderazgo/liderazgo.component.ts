...
export class LiderazgoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.cargar();
    this.search.valueChanges.pipe(debounceTime(200)).subscribe(() => this.filtrar());
  }
  ...
  private filtrar() {
    ...
    this.filtrados = q ? this.liderazgos.filter(x => x.nombre.toLowerCase().includes(q)) : this.liderazgos.slice();
  }
  ...
  agregar() {
    ...
    this.svc.crearLiderazgo(n).subscribe({
      next: () => { this.nombreNuevo.reset(''); this.cargar(); Swal.fire('Creado','Ministerio creado','success'); }
    });
  }
...
openMembers(m: { id: number; nombre: string }) {
  ...
  editar(min: Liderazgo) {
    this.dialog.open(DialogEditMinistryComponent, {
      width: '560px', disableClose: true, data: { liderazgo: min }
    }).afterClosed().subscribe(ok => { if (ok) this.cargar(); });
  }

  eliminar(min: Liderazgo) {
    Swal.fire({title:'¿Eliminar ministerio?', icon:'question', showCancelButton:true, confirmButtonText:'Sí'})
      .then(r => {
        if (r.isConfirmed) {
          this.svc.eliminarLiderazgo(min.id).subscribe({
            next: () => { Swal.fire('Eliminado','Ministerio eliminado','success'); this.cargar(); }
          });
        }
      });
  }
